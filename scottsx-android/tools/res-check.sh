...
# -- 0. XML comments must not contain "--" ---------------------------------
# aapt2's merger hard-fails on it (this burned a CI cycle: values/colors.xml
# comment mentioning a CSS token like --brand-400). Sweep res/*.xml + manifest.
comment_bad=0
...
for m in re.finditer(r"<!--(.*?)-->", text, re.S):
    if "--" in m.group(1):
        ...
        print(f"{sys.argv[1]}:{line}: double hyphen inside XML comment")
        bad = True
sys.exit(1 if bad else 0)
' "$f"; then :; else comment_bad=1; fi
done < <(find "$RES" -name "*.xml" -type f; echo "$MANIFEST")
[ "$comment_bad" -eq 0 ] && ok "no double hyphens inside XML comments" \  || bad "double hyphen inside an XML comment (aapt2 refuses to merge)"
...
notif_icon="$(grep -oE 'default_notification_icon"[[:space:]]*$|@drawable/[A-Za-z0-9_]+' "$MANIFEST" \
              | grep -A0 'drawable' | tail -1 | sed 's|@drawable/||' || true)"
notif_kt="$(grep -rhoE 'setSmallIcon\(R\.drawable\.[A-Za-z0-9_]+' "$SRC" | sed 's/.*\.//' | sort -u || true)"
...
for ai in "$RES"/mipmap-anydpi-v26/*.xml; do
  ...
  fg="$(grep -oE '<foreground[^>]*android:drawable="@[a-z]+/[A-Za-z0-9_]+"' "$ai" | sed 's|.*/||; s|"||' || true)"
  bg="$(grep -oE '<background[^>]*android:drawable="@[a-z]+/[A-Za-z0-9_]+"' "$ai" | sed 's|.*/||; s|"||' || true)"
  if [ -n "$fg" ]; then
